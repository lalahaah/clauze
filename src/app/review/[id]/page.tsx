...
function GridSVG({ gridOffX, gridOffY }: {
  ...
  return (
    <svg style={{ width: "100%", height: "100%" }}>
      <defs>
        <motion.pattern id="gp-review" width="40" height="40" patternUnits="userSpaceOnUse" x={gridOffX} y={gridOffY}>
...
const DEMO_REVIEW: Review = {
  ...
  storageUrl: "", processingTime: 28000,
  riskLevel: "high", createdAt: new Date().toISOString(),
  result: {
    overallRisk: "high",
  ...
  useEffect(() => {
    ...
    if (stored) {
      try {
        ...
        if (Array.isArray(data.repeatedPatterns)) setRepeatedPatterns(data.repeatedPatterns);
      } catch { /* sessionStorage 파싱 실패 시 DEMO_REVIEW 유지 */ }
    }
  }, [id]);
    ...
    <div style={{ background: R.bgLight, minHeight: "100vh", fontFamily: R.fontSans, overflowX: "hidden" }}>
      <style>{`
        * { box-sizing: border-box; }
        .review-utility { padding: 6px 40px; }
        .review-nav { padding: 0 40px; gap: 24px; }
        .review-hero { padding: 64px 40px 108px; }
        .review-hero-inner { max-width: 1100px; margin: 0 auto; }
        .review-content { max-width: 1100px; margin: -48px auto 0; padding: 0 40px 40px; position: relative; z-index: 20; }
        .review-footer-grid { display: grid; grid-template-columns: 2fr 1fr 1fr 1fr; gap: 48px; margin-bottom: 48px; }
        .review-footer-pad { padding: 64px 40px 32px; }
        .review-user-email { display: block; }
        .review-filename { white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        @media (max-width: 768px) {
          .review-utility { padding: 6px 16px; }
          .review-nav { padding: 0 16px; gap: 12px; }
          .review-hero { padding: 40px 20px 80px; }
          .review-content { padding: 0 20px 40px; margin: -32px auto 0; }
          .review-footer-grid { grid-template-columns: 1fr 1fr; gap: 24px; margin-bottom: 32px; }
          .review-footer-pad { padding: 48px 20px 24px; }
          .review-user-email { display: none; }
          .review-filename { white-space: normal; word-break: break-word; font-size: clamp(16px, 4vw, 28px) !important; }
          .review-back-btn { display: none; }
        }
        @media (max-width: 480px) {
          .review-footer-grid { grid-template-columns: 1fr; }
        }
      `}</style>

      {/* Top utility bar */}
      <div className="review-utility" style={{ background: R.bgDark, display: "flex", justifyContent: "flex-end", alignItems: "center", gap: 24 }}>
        {user && !loading && (
          <div className="review-user-email" style={{ fontSize: 11, fontWeight: 600, color: "rgba(255,255,255,0.7)", fontFamily: R.fontSans }}>{user.email}</div>
        )}
        <button onClick={() => user ? logout() : router.push("/login")} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 11, fontWeight: 700, letterSpacing: "1.2px", color: "rgba(255,255,255,0.7)", fontFamily: R.fontSans, textTransform: "uppercase" }}>
          {user ? "LOGOUT" : "LOGIN"}
        </button>
      ...
      {/* Sticky nav */}
      <nav className="review-nav" style={{ background: R.bgWhite, borderBottom: `1px solid ${R.borderLight}`, display: "flex", alignItems: "center", height: 68, position: "sticky", top: 0, zIndex: 100 }}>
        <Link href="/" style={{ display: "flex", alignItems: "center", gap: 10, marginRight: 8, textDecoration: "none", flexShrink: 0 }}>
        ...
        {/* Breadcrumb */}
        <span style={{ fontSize: 13, color: R.textLight, fontFamily: R.fontSans, whiteSpace: "nowrap" }}>
          <span onClick={() => router.push("/dashboard")} style={{ cursor: "pointer", color: R.textMid }}>Dashboard</span>
          {" / "}
        ...
        <button className="review-back-btn" onClick={() => router.push("/dashboard")} style={{ background: "none", border: `1px solid ${R.borderLight}`, borderRadius: R.btnRadius, padding: "7px 18px", fontSize: 13, color: R.textMid, fontFamily: R.fontSans, cursor: "pointer", fontWeight: 600, whiteSpace: "nowrap", marginLeft: 12 }}>← Dashboard</button>
      </nav>

      {/* Dark hero */}
      <div className="review-hero" onMouseMove={e => { mouseX.set(e.clientX); mouseY.set(e.clientY); }} style={{ position: "relative", background: R.bgDark, overflow: "hidden" }}>
        ...
        <div className="review-hero-inner" style={{ position: "relative", zIndex: 10 }}>
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.45 }}>
            ...
            <h1 className="review-filename" style={{ fontFamily: R.fontSans, fontSize: "clamp(18px, 3vw, 36px)", fontWeight: 800, color: R.textWhite, letterSpacing: "-0.02em", lineHeight: 1.2, margin: "0 0 20px", maxWidth: 700 }}>
              {review.fileName}
            </h1>
      ...
      {/* Footer */}
      <footer className="review-footer-pad" style={{ background: R.bgDark }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div className="review-footer-grid">
            <div>
