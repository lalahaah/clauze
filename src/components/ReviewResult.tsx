  ...
  return (
    <>
      ...
      <div className="review-result-wrap">
        {/* Left panel */}
        <aside className="review-result-aside">

          {/* Risk summary */}
          <div style={{ background: "#FFFFFF", padding: "24px", borderRadius: 4, borderTop: "3px solid #D94F3D" }}>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "1.5px", color: "#7A9A9E", marginBottom: 14, textTransform: "uppercase", fontFamily: "'DM Sans', sans-serif" }}>
              {lang === "ko" ? "위험 요약" : "Risk Summary"}
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              {[
                { n: highCount, l: lang === "ko" ? "고위험" : "High", c: "#D94F3D", bg: "rgba(217,79,61,0.08)" },
                { n: mediumCount, l: lang === "ko" ? "주의" : "Caution", c: "#E59A1A", bg: "rgba(229,154,26,0.08)" },
                { n: lowCount, l: lang === "ko" ? "정상" : "Normal", c: "#1A9E6A", bg: "rgba(26,158,106,0.08)" },
              ].map(({ n, l, c, bg }) => (
          ...
          <div style={{ background: "#FFFFFF", padding: "20px 24px", borderRadius: 4 }}>
            ...
            <p style={{ fontSize: 13, color: "#3D5A5E", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", fontFamily: "'DM Sans', sans-serif", margin: 0 }} title={review.fileName}>{review.fileName}</p>
            <p style={{ fontSize: 12, color: "#7A9A9E", marginTop: 4, fontFamily: "'DM Mono', monospace", margin: "4px 0 0" }}>
          ...
          {/* Lawyer CTA */}
          {highCount > 0 && (
            <button style={{ width: "100%", padding: "14px", background: "#00A599", color: "#FFFFFF", border: "none", borderRadius: 28, fontSize: 14, fontWeight: 700, fontFamily: "'DM Sans', sans-serif", cursor: "pointer" }}>
              변호사 연결 상담 →
            </button>
          )}

          <p style={{ fontSize: 11, color: "#7A9A9E", textAlign: "center", lineHeight: 1.6, fontFamily: "'DM Sans', sans-serif" }}>
            ※ 본 검토는 참고용이며 법적 효력이 없습니다.
          </p>
        </aside>

        {/* Clause list */}
        <main className="review-result-main">
